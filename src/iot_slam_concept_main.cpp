...
int main(int argc, char* argv[]) {
  ...
  // generate SLAM sessions for each floor
  std::vector<double> init_z{1, 11};
  for (const auto& [floor_id, floor_anchor_association] :
       floor_anchor_association_map) {
    // generate ground truth and measured robot positions
    gtsam::Pose3 init_pose = gtsam::Pose3(
        gtsam::Rot3::Ypr(0, 0, 0), gtsam::Point3(10, 0, init_z.at(floor_id)));
    ...
    // output to txt files
    PrintPoses(ground_truth_robot_poses, full_file_path +
                                             "ground_truth_robot_poses_" +
                                             std::to_string(floor_id) + ".txt");
    PrintPoses(measured_robot_poses, full_file_path + "measured_robot_poses_" +
                                         std::to_string(floor_id) + ".txt");
    ...
    //   initial.insert<gtsam::Point3>(gtsam::Symbol('t', tag_id),
    //   tag_position);
    // }
    ...
    PrintPoints(optimized_robot_points, full_file_path +
                                            "optimized_robot_points_" +
                                            std::to_string(floor_id) + ".txt");
    ...
    PrintMap(optimized_anchor_position_map,
             full_file_path + "optimized_anchor_position_map_" +
                 std::to_string(floor_id) + ".txt");
